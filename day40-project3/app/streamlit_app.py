...
def stream_chat(message:str,thread_id:str):
    with httpx.stream("POST",f"{API_BASE}/api/chat/stream",json = {"message":message,"thread_id":thread_id},timeout = 60.0) as r:
            ...
            with st.spinner("思考中..."):
                try:
                    for event in stream_chat(user_input,st.session_state.thread_id):
                        # 1. 工具调用（如果后端传）
                        if "tool_calls" in event and event["tool_callls"]:
                            tool_calls_in_run.extend(event["tool_calls"])
                        # 2. 工具结果（如果后端传）
