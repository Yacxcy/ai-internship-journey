...
# 从一段文本中抽取结构化信息（姓名、电话、地址等）
def extract_info(text :str ,fields :list) ->dict:
    """从文本中按字段抽取结构化信息"""
    prompt = f""" 从下面文本中抽取这些字段并输出 JSON:{fields}
            文本: {text}
            只输出 JSON，找不到的字段值设为 null。"""
    ...
    return json.loads(resp.choices[0].message.content) # 解析返回的 JSON 字符串并返回一个字典
